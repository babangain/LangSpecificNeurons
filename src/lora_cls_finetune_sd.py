...
class LoRAFineTuner:
    def __init__(self, device: torch.device, config: dict):
        ...
        self.run_name = f"{self.lang_set}_{self.lang}_finetune_{self.finetune_lang}_{self.config['train_frac']:.2f}_{self.config['initial_lr']:.1e}_r{self.config['lora_rank']}"
        self.output_dir = f"outputs/ckpt/{self.project_name}/{self.run_name}"

        self.frozen_neurons = LoRAFineTuner._get_frozen_neurons(model_name=self.model_name.split("/")[-1], finetune_lang=self.finetune_lang, lang_set=self.lang_set)
    ...
    def _get_frozen_neurons(model_name: str, lang_set: str, finetune_lang: str) -> torch.tensor:
        ...
        if finetune_lang == "null":
            return None
        elif "+" in finetune_lang:
            lang1, lang2 = finetune_lang.split("+")
        ...
        elif len(finetune_lang) == 2:
            ft_index = lang_neuron["lang_to_neuron"][finetune_lang] # (N, 2)
        else:
...
def main(model_name: str, device: torch.device) -> None:
    config = {
        ...
        "lang_set": "set1", "lang": "en", "finetune_lang": "null", # ["en", "vi", "en+vi", "null"]
