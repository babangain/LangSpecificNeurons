...
class LoRAFineTuner:
    def __init__(self, device: torch.device, config: dict):
        ...
        self.run_name = f"{self.method}_data_{self.lang}_frozen_{self.frozen_lang}_{self.config['train_frac']:.2f}_{self.config['initial_lr']:.1e}_r{self.config['lora_rank']}"
    ...
    def _get_frozen_neurons(self) -> torch.tensor:
        lang_neuron_path = Path(Path.cwd(), f"outputs/lang_neurons/{self.model_name.split('/')[-1]}/{self.method}/lang_neuron_data.pkl")
        ...
        index = lang_neuron["lang_to_neuron"][self.frozen_lang] # (N, 2)
        return index
