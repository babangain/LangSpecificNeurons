...
from transformers import AutoTokenizer, TrainingArguments, Trainer, DefaultDataCollator, get_cosine_with_hard_restarts_schedule_with_warmup, get_linear_schedule_with_warmup, BitsAndBytesConfig, TrainerCallback, QuantoConfig
from dataset import WikipediaDatasetHF
from utils import models_map
from sft_models import ModelForMLMWithSFT
import evaluate
import numpy as np
...
class SFTFineTuner:
    def __init__(self, device: torch.device, config: dict):
        self.config = config
        self.device = device        
        self.quant_config = QuantoConfig(
            weights="float8", 
            activations=None,
        ...
        self.train_ds = Subset(self.ds, indices=random.sample(range(len(self.ds)), k=int(len(self.ds) * config['data_frac'])))
        self.eval_ds = Subset(self.train_ds, indices=random.sample(range(len(self.train_ds)), k=int(len(self.train_ds) * 0.1)))
        ...
        self.run_name = f"data_{self.lang}_round_{self.config['sft_round']}_{self.config['data_frac']:.2f}_{self.config['initial_lr']:.1e}"
        self.output_dir = f"outputs/ckpt/{self.project_name}/{self.run_name}"

        self.model = ModelForMLMWithSFT(device=self.device, tokenizer=self.tokenizer, model_name=self.model_name, quant_config=self.quant_config, neurons_frac=config['neurons_frac'])
        if self.config["sft_round"] == 1:
            self.model.prepare_for_round1_sft()
        elif self.config["sft_round"] == 2:
            self.model.prepare_for_round2_sft()
        ...
        # self.optimizer = bnb.optim.Adam8bit(params=self.model.parameters(), lr=self.config['initial_lr'], weight_decay=self.config["weight_decay"], betas=self.config["adam_betas"])
    ...
    def compute_loss(self, model: torch.nn.Module, inputs: dict, outputs: torch.tensor = None):
        """Not to be used by SFTFineTuner.
    ...
    def load_model(device: torch.device, config_path: Path, checkpoint_name: str) -> dict:
        ...
        model = ModelForMLMWithSFT(device=device, tokenizer=tokenizer, model_name=model_name, quant_config=config_data["quant_config"], neurons_frac=config['neurons_frac'])
...
def main(model_name: str, device: torch.device) -> None:
    config = {
        "model_name": model_name, "task_name": "SFT", "sft_round": 1,
        "lang": "en",
        "num_epochs": 1, "num_steps": None, "batch_size": 4, "max_context_length": 512, # steps are auto calculated
        "data_frac": 0.5, "neurons_frac": 0.0005,
        "initial_lr": 1e-6, "max_grad_norm": 10.0, "weight_decay": 0.01,
        "adam_betas": (0.95, 0.999), "grad_acc_steps": 16, "num_ckpt_per_epoch": 4, "is_quant": True, "fp16": False, "bf16": True,
        "wandb_log": False
    }
    trainer = SFTFineTuner(device=device, config=config)
    print(trainer.model)
    trainer.train()
